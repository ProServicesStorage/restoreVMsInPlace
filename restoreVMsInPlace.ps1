...
$response = Invoke-RestMethod 'http://commserve1.cv.lab/webconsole/api/Login' -Method 'POST' -Headers $headers -Body $body
...
$ccVms = Invoke-RestMethod 'http://commserve1.cv.lab/webconsole/api/VM' -Method 'GET' -Headers $headers
...
foreach ($vm in $vms) {
    ...
    if ($null -ne $vmGuid) {
        ...
        $response = Invoke-RestMethod "http://commserve1.cv.lab/webconsole/api/v2/vsa/vm/$vmGuid/recover" -Method 'POST' -Headers $headers -Body $body -ContentType 'application/xml'
